...
const STOOL_REPO = 'fufengyuan/supertool';
const STOOL_INSTALL_DIR = join(homedir(), '.dsh', 'bin');
const STOOL_BIN = process.platform === 'win32' ? 'stool.exe' : 'stool';
const STOOL_PATH = join(STOOL_INSTALL_DIR, STOOL_BIN);
...
async function runStool(args) {
  ...
  const stoolPath = globalThis.__dsh_stool_path || 'stool';
...
async function ensureStoolInstalled(ctx) {
  const subprocess = ctx.get('subprocess');
  if (!subprocess) return false;

  // 1. Check if stool is in PATH
  try {
    ...
    if (r.exitCode === 0) {
      globalThis.__dsh_stool_path = 'stool';
      return true;
    }
  } catch (e) { /* ignore */ }

  // 2. Check if already installed to ~/.dsh/bin
  try {
    const h = subprocess.spawn({
      argv: ['test', '-x', STOOL_PATH],
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1024 }, stderr: { maxBytes: 1024 } },
      graceMs: 5000,
    });
    const r = await h.done;
    if (r.exitCode === 0) {
      globalThis.__dsh_stool_path = STOOL_PATH;
      return true;
    }
  } catch (e) { /* ignore */ }

  // 3. Auto-install: download from GitHub releases
  try {
    const osMap = { darwin: 'darwin', linux: 'linux', win32: 'windows' };
    const archMap = { arm64: 'arm64', x64: 'amd64' };
    const os = osMap[process.platform];
    const cpu = archMap[process.arch];
    if (!os || !cpu) return false;

    const url = 'https://github.com/' + STOOL_REPO + '/releases/latest/download/stool-' + os + '-' + cpu;

    // Create install dir
    await subprocess.spawn({
      argv: ['mkdir', '-p', STOOL_INSTALL_DIR],
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1024 }, stderr: { maxBytes: 1024 } },
      graceMs: 5000,
    }).done;

    // Download with curl
    const dl = subprocess.spawn({
      argv: ['curl', '-fsSL', '-o', STOOL_PATH, url],
      stdio: { stdin: 'ignore', stdout: { maxBytes: 4096 }, stderr: { maxBytes: 1024 * 1024 } },
      graceMs: 60000,
    });
    const dlResult = await dl.done;
    if (dlResult.exitCode !== 0) return false;

    // Make executable
    await subprocess.spawn({
      argv: ['chmod', '+x', STOOL_PATH],
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1024 }, stderr: { maxBytes: 1024 } },
      graceMs: 5000,
    }).done;
...
export async function apply(ctx) {
  ...
  if (subprocess) {
    ...
    // Auto-install stool if not found
    if (!await ensureStoolInstalled(ctx)) {
      console.warn('[dsh-stool] stool CLI not found — install from https://github.com/' + STOOL_REPO + ' or tools will fail when called');
    }
  }
