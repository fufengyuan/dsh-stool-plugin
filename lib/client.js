...
window.__ModuleLoader__.load({
  ...
  factory: function(require) {
    ...
    var TOOLS = [
      { name: 'stool_server', icon: '🖥️', desc: '服务器管理', actions: 'list / test / exec / exec-batch / health / diagnose / read / ls / download / upload / mkdir / rm / java-ps / java-restart' },
      { name: 'stool_db', icon: '🗄️', desc: '数据库与 Redis', actions: 'list / query / databases / tables / structure / data / redis(13 个子命令)' },
      { name: 'stool_log', icon: '📋', desc: '日志查询', actions: 'list / search / tail / context / add / delete' },
      { name: 'stool_cicd', icon: '🚀', desc: 'CI/CD 部署', actions: 'list / status / deploy / history / step-logs / rollback / cancel / modules / logs / tools' },
      { name: 'stool_mfa', icon: '🔐', desc: 'MFA 认证', actions: 'list / code / codes / add / delete / parse-uri' },
      { name: 'stool_git', icon: '📦', desc: 'Git 仓库', actions: 'list / status / log / branches / pull / push / commit / checkout' },
      { name: 'stool_todo', icon: '✅', desc: '待办任务', actions: 'list / add / complete / uncomplete / delete / show / edit / search / stats / clear' },
      { name: 'stool_note', icon: '📝', desc: '笔记管理', actions: 'list / add / update / delete / groups / add-group / update-group / delete-group' },
      { name: 'stool_project', icon: '📁', desc: '项目管理', actions: 'list / add / show / update / delete / stats / todos' },
      { name: 'stool_subtask', icon: '☑️', desc: '子任务', actions: 'list / add / complete / delete' },
      { name: 'stool_nginx', icon: '🌐', desc: 'Nginx 配置', actions: 'list / add / update / delete / fetch / test / deploy / versions' },
      { name: 'stool_misc', icon: '🔧', desc: '记账 / 周报 / 审计', actions: 'accounting / trend / weekly / weeklyShow / weeklySave / audit' }
    ];
      ...
      return h('div', {
        ...
        h('button', {
          ...
          h('div', { style: { flex: 1, minWidth: 0 } },
            ...
            }, '通过 ' + TOOLS.length + ' 个合并工具覆盖 stool 全部能力（服务器/数据库/日志/CI-CD/Git/MFA/待办/笔记/项目/子任务/Nginx/记账审计），Agent 可自动发现并调用。')
          ),
          statusChip(kind),
          chevron(open)
        ),
